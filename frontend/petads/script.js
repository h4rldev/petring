...
async function getRandomAd() {
	...
	fetch(`${api_url}/get/random-ad`)
		...
		.then((data) => {
			...
			document.title = ad_data.username;
			image_element.src = ad_data.image_url;
			image_element.alt = ad_data.username;
			image_link_element.href = ad_data.ad_url;
			promo_link_element.innerText = `from ${ad_data.username} (click here for more info about PetAds)`;
		})
		.catch(async (error) => {
			if (error instanceof Response) {
				const json = await error.json();
				const message = `${error.status}: ${json.message}`;
