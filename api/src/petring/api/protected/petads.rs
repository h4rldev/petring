use crate::petring::api::BulkAdsResponse;
...
pub async fn get_ad(
  ...
  if !ad.verified {
    return petring_api_err(StatusCode::NOT_FOUND, "Ad not verified");
  }
...
pub async fn get_unverified_ads(State(state): State<AppState>) -> impl IntoResponse {
  let ads = match Ads::find()
    .filter(ads::Column::Verified.eq(false))
    .all(&state.db)
    .await
  {
    Ok(ads) => ads,
    Err(err) => {
      return petring_api_err(
        StatusCode::INTERNAL_SERVER_ERROR,
        &format!("Failed to fetch ads: {err}"),
      );
    }
  };

  let bulk_ads_response = BulkAdsResponse {
    ads: ads
      .iter()
      .map(|ad| AdResponse {
        username: ad.username.clone(),
        ...
        edited_at: ad.edited_at.clone(),
        verified_at: ad.verified_at.clone(),
      })
      .collect(),
  };

  petring_api_response(StatusCode::OK, bulk_ads_response)
}
...
pub async fn delete_ads_by_bulk(
  State(state): State<AppState>,
  ...
  if let Some(discord_ids) = bulk_delete_request.discord_ids {
    let discord_ids: Vec<i64> = discord_ids.into_iter().map(|x| x as i64).collect();
    let ads = match Ads::find()
      .filter(ads::Column::DiscordId.is_in(discord_ids))
      .all(&state.db)
      .await
    {
      Ok(ads) => ads,
    ...
    ads_to_delete.extend(ads);
  }

  if let Some(usernames) = bulk_delete_request.usernames {
    let ads = match Ads::find()
      .filter(ads::Column::Username.is_in(usernames))
      .all(&state.db)
      .await
    {
      Ok(ads) => ads,
