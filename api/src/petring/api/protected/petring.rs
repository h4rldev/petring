use super::{
  BulkUserDeleteRequest, BulkUserDeleteResponse, BulkUsersResponse, EditUserResponse, UserEdit,
...
pub async fn get_unverified_users(State(state): State<AppState>) -> impl IntoResponse {
  let users = match Users::find()
    .filter(users::Column::Verified.eq(false))
    .all(&state.db)
    .await
  {
    Ok(users) => users,
  ...
  let bulk_users_response = BulkUsersResponse {
    users: users
      .iter()
      .map(|user| UserResponse {
        ...
        edited_at: user.edited_at.clone(),
        verified_at: user.verified_at.clone(),
      })
      .collect(),
  };

  petring_api_response(StatusCode::OK, bulk_users_response)
}
...
pub async fn delete_users_by_bulk(
  State(state): State<AppState>,
  ...
  if let Some(discord_ids) = bulk_delete_request.discord_ids {
    ...
    let users = match Users::find()
      .filter(users::Column::DiscordId.is_in(discord_ids))
      .all(&state.db)
      .await
    {
      Ok(users) => users,
